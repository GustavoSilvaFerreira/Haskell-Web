...
|] >> toWidget [julius|
	...
	function confeditVagaValor(){
        ...
        $.ajax({
            ...
            data: JSON.stringify(modeledt),
            success: function(){
				$("#alteracao").html("Histórico de alteração atualizado com sucesso!<br>");
   		...
   		$.ajax({
            ...
            type: "GET",
       		...
       		for(var i = 0; i<e.data.length; i++){
           		...
           		itens+="</span></td>"
       	    	itens+="<td><span>"
           		itens+=e.data[i].vldiurnoantigo.toFixed(2);
           		itens+="</span></td>"
            	itens+="<td><span>"
           		itens+=e.data[i].vldiurnonovo.toFixed(2);
             	itens+="</span></td>"
             	itens+="</span></td>"
       	    	itens+="<td><span>"
           		itens+=e.data[i].vlnoturnoantigo.toFixed(2);
           		itens+="</span></td>"
       	    	itens+="<td><span>"
           		itens+=e.data[i].vlnoturnonovo.toFixed(2);
           		itens+="</span></td>"
            	itens+="<td><span>"
           		itens+=e.data[i].funcionarioid;
             	itens+="</span></td>"
